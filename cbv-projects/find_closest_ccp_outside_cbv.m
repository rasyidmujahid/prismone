...
function closest_ccp = find_closest_ccp_outside_cbv(point, roughing_points, boundary_points)
	neighbour_points = find_neighbour_points(point, roughing_points);
	ccp_outside_cbv = find_neighbour_outside_cbv(neighbour_points, boundary_points);
	closest_ccp = ccp_outside_cbv;
end
...
function outputs = find_neighbour_points(point, roughing_points)
	outputs = [];

	%% neighbour along X axis: find all points with the same X and Z
	neighbour_x = sortrows(...
		roughing_points(...
			float_equals(roughing_points(:,1), point(1)) & float_equals(roughing_points(:,3), point(3)),
			:), 
		2);
	if ~are_all_under_cbv(neighbour_x)
		outputs = neighbour_x;
	end
	...
	neighbour_y = sortrows(...
		roughing_points(...
			float_equals(roughing_points(:,2), point(2)) & float_equals(roughing_points(:,3), point(3)),
			:), 
		1);
	if ~are_all_under_cbv(neighbour_y)
	...
	neighbour_z = sortrows(...
		roughing_points(...
			float_equals(roughing_points(:,1), point(1)) & float_equals(roughing_points(:,2), point(2)),
			:), 
		3);
	if ~are_all_under_cbv(neighbour_z)
...
%% find_neighbour_outside_cbv: given an array of neighbour points, find one that 
%%		fall outside cbv.
%% 
function outputs = find_neighbour_outside_cbv(neighbour_points, boundary_points)
	outputs = [];
end
